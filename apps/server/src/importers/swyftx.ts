// Swyftx order history CSV importer.
//
...
//   buy  → Total AUD × 100  (total paid, includes Swyftx spread/fee = cost base)
//   sell → Total AUD × 100  (net received after spread/fee = CGT proceeds)
//
...
export interface SwyftxImportError {
  file: string;
  row: number;
  message: string;
}
...
export interface SwyftxImportPreview {
  filename: string;
  total: number;
  new_count: number;
  duplicate_count: number;
  trades: CryptoPreviewTrade[];
  errors: SwyftxImportError[];
}

export interface SwyftxCommitResult {
  inserted: number;
...
function parseSwyftxDate(raw: string): string {
...
export function buildSwyftxPreview(files: readonly InputFile[]): SwyftxImportPreview[] {
  return files.map((f) => parseFile(f));
}

function parseFile(f: InputFile): SwyftxImportPreview {
  const errors: SwyftxImportError[] = [];
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    try {
      ...
      const tradeDate = parseSwyftxDate(dateRaw);
      ...
      const extId = `swyftx-${sha1Short(rowSeed)}`;
      ...
      if (isCoinToCoin) {
        ...
        const buyExtId = `swyftx-${sha1Short(rowSeed + '-buy')}`;
...
export function commitSwyftxImport(previews: SwyftxImportPreview[]): SwyftxCommitResult {
  const result: SwyftxCommitResult = { inserted: 0, skipped_duplicates: 0, new_assets: 0 };
